...
- Run the test suite; ensure all three tests pass.
- Manually grep the source for the TTL constant to satisfy the regex check.

## Risks
- Deviation from existing validator integration may require coordination with scoped_tokens logic.
- Over‑deletion of code is prevented by adding a new file rather than modifying existing validator.
