...
## Verification
Validated via pytest suite and grep check.

## Risks
- Over‑deleting existing content in `scoped_tokens.py`; we only appended a helper function.
- Ensure the new import does not cause circular import issues (module layout is flat within the package).
- Tests rely on real time; small clock drift is acceptable as we use a 1‑hour recent window.
