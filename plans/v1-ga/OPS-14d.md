# OPS-14D: Enforce 24h TTL constraint on scoped OAuth2 tokens
...
Run `pytest -q tests/auth/test_ttl_validation.py` expecting exit code 0 and verify grep finds the TTL constant in the source.

## Risks
- Potential breakage of existing token flows if validator is invoked incorrectly.
- Added dependency on `fastapi.HTTPException` in a utility module.
- Slight performance overhead from time check on each token validation.
