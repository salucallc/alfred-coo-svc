...
- Run `pytest -q tests/auth/test_ttl_validation.py` ensuring exit code 0 and all three tests pass.
- Verify the TTL constant `86400` appears in `src/alfred_coo/auth/ttl_validator.py` via grep.

## Risks
- If existing token flow elsewhere expects different error handling, integration may need updates.
- Adding a new module may require import adjustments; tests import directly to avoid side effects.
- Over‑deleting code in `scoped_tokens.py` is prohibited; we only add a sibling module.
