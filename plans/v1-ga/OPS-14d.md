...
Run pytest suite; ensure all three tests pass. Verify TTL constant present via grep.

## Risks
Potential impact on existing token issuance flows; ensure backward compatibility by only validating during token usage, not issuance.
