...
## Verification
Run the pytest suite; ensure all three tests pass. Confirm the constant `TTL_SECONDS` (86400) appears in the source via grep.

## Risks
- Deleting or altering existing scaffolding in `scoped_tokens.py` could break dependent code; we only add a new module.
- Hard‑coding the error body may need alignment with downstream response format.
- Future token validation integration must import and invoke `validate_iat` correctly.
