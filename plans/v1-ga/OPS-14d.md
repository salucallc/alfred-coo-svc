...
Added unit tests using `freezegun` to freeze time and verify the validator behaviour. Running `pytest` ensures all criteria are met.

## Risks
* Changing token validation could affect other auth flows – mitigated by keeping existing scaffold untouched and adding a dedicated module.
* Introducing `freezegun` as a test dependency – already used in other tests.
