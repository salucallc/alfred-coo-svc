...
- Added TTL validator module and integrated it with scoped_tokens.
- Created comprehensive tests asserting correct behaviour for valid, expired, and missing iat cases.
- Verified TTL constant presence via grep.

## Risks
- No impact on existing token issuance flow; validation only occurs on token consumption.
- Minimal code addition, low performance overhead.
