...
## Verification
All tests pass and TTL constant is present in the validator module.

## Risks
- Deleting or overwriting existing validator logic could breach the 70% deletion guard; we add a new sibling module to avoid extensive rewrites.
- Ensure deployment integrates `ttl_validator.validate_iat` into request flow; out‑of‑scope for this ticket but documented for follow‑up.
