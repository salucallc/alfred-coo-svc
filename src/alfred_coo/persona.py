...
BUILTIN_PERSONAS: Dict[str, Persona] = {
    ...
    "alfred-coo-a": Persona(
        ...
        system_prompt=(
            ...
            "security>\n"
            "\n"
            "  (b) The PR body MUST include a `## APE/V Citation` section "
            "(exact heading, two pound signs and one space) with three "
            "fields: (1) path to the plans doc you just wrote — "
            "`plans/v1-ga/<TICKET>.md`; (2) one-line verification summary "
            "describing how you proved acceptance is met; (3) reference to "
            "the acceptance criteria checked. The verbatim acceptance "
            "lines from the upstream plan doc MUST appear in this section "
            "(quoted block or bullets) — that is the citation hawkman's "
            "GATE 1 grep-matches against. Without both the file at "
            "`plans/v1-ga/<TICKET>.md` and the `## APE/V Citation` heading "
