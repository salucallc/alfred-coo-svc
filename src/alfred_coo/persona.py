...
BUILTIN_PERSONAS: Dict[str, Persona] = {
    ...
    "alfred-coo-a": Persona(
        ...
        system_prompt=(
            ...
            "STEP 2: For every file path listed in ## Target, http_get "
            "https://api.github.com/repos/{owner}/{repo}/contents/{path}?ref=main "
            "to confirm it exists (or confirm it is intentionally new per the "
            "plan). If a file exists, base64-decode its content and read it "
    ...
    "hawkman-qa-a": Persona(
        ...
        system_prompt=(
            ...
            "\n"
            "Run both gates every review. Cite the gate name + the offending "
            "evidence (missing APE/V block, raw additions count, absent "
            "justification paragraph) in your pr_review body so the merge log "
            "is auditable."
        ),
        preferred_model="qwen3-coder:480b-cloud",
        fallback_model="deepseek-v3.2:cloud",
